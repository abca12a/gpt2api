...
- 单个 free 账号：不能承诺单次 ChatGPT 会话稳定一次性出满 4 张；生产多图依赖当前并发拆单策略和 `result.data[]` 聚合。
...
- 分辨率路由：不能让 `1k` 继续默认尝试 Codex/APIMart；当前 `1k` 是 runner-only，`2k/4k` 才优先外置渠道。
- 瞬态兜底：不能只把 HTTP 502 当作可兜底错误；`5xx`、timeout、EOF、connection reset、broken pipe 等都应按瞬态处理。
...
- 4K 预期：ChatGPT Web `picture_v2` 链路没有稳定可控的原生 2K/4K 字段；原生大图主要依赖 Codex image channel / APIMart，Web Runner 只能作为默认档位或完成率兜底。
...
- 多图完整性：不能用下游 `private_data.result_url` 或列表兼容字段判断多图是否丢失；必须读取任务详情 `result.data[]`。
- 分档计费：不要把号池内部 `credit_cost=10` 当成下游用户实际扣费；用户侧真相源是下游 `tasks.quota`、`logs.other.model_price` 和 `private_data.billing_context`。

## gpt-image-2 后台模型配置

- 曾经误解：把 han 要求的“按照请求体参数配置”理解成把 `gpt-image-2:1k/2k/4k` 暴露成多个后台模型项，后来又把中间回退误当成最终状态。
- 当前纠正：后台真实模型仍应是单个 `gpt-image-2`；但下游后端可以按请求体 `resolution` 返回 `/api/pricing.resolution_options` 并在任务创建时按分辨率计费。
- 当前默认：`1k=0`、`2k=0.06`、`4k=0.12` ⭐，并按 `n` 放大；旧订单复核要传订单创建时价格，不能拿当前价格直接判错。
- 边界：`gpt-image-2:1k/2k/4k` 可以作为后台覆盖键，但不要把它们当成多个真实模型行或让前端自行推导价格。
...
- 宿主预编译：不能只执行 `docker compose build/up` 就认为已经部署了当前源码；本项目镜像只复制已有 `deploy/bin/gpt2api`。
- Docker 网络：不能只看 `cli-proxy-api` 容器是 `Up` 就判断外置图片渠道正常；如果它没加入 `deploy_default`，`gpt2api-server` 容器内会解析不到 `cli-proxy-api`。
- 图片签名：`JWT_SECRET` 不能为空，且不要随意轮换；它同时影响登录 JWT 和 `/p/img` 签名派生，换密钥会让旧签名 URL 失效。
- 机器拓扑：当前 Codex 是号池，不是下游后端；涉及 IP、用户、项目目录、构建机职责时先看 `AGENTS.md`。
