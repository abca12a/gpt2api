...
- 具体 SSH 端口、私钥路径、agent 状态以当前 SSH 配置或当次验证为准；不要把一次性端口验证写成长期事实。当前 Codex 环境已验证：若 `~/.ssh/cliproxyapi_212_50_232_214_ed25519` 存在，优先用它进入下游后端、下游前端和构建机，不要先做无 `IdentityFile` 的默认 SSH 探测。
