...
> 基于逆向 **chatgpt.com** 的 OpenAI 兼容 SaaS 网关 —— 多账号池 / 代理池 / **IMG2 终稿直出** / **批量出图** / **高并发调度** / 积分计费 / 管理后台一体化。
...
- [八、重点能力详解](#八重点能力详解)
  - [8.1 IMG2 终稿直出测试](#81-img2-终稿直出测试)
...
> 本项目当前版本**聚焦图片模型**(详见 [8.1 IMG2 终稿直出测试](#81-img2-终稿直出测试) 与 [8.2 批量出图](#82-批量出图--多张聚合))。文字通路(`/v1/chat/completions`)代码层完整保留,但因 `chatgpt.com` 新 sentinel 协议存在短期不稳定因素,UI 入口已在当前版本关闭,恢复只需改一行 feature flag,详见 [十一、二次开发](#十一二次开发--定制)。
...
| **图片生成** | 文生图、**图生图 / 多图参考**、**IMG2 正式版直出**(速度优先,SSE 够数即返回,60s 短轮询补齐)、轮询 + SSE 直出双通道 |
