...
3. 启动 HTTP 服务 `:8080`(由 nginx 反代到 `80/443`)。
...
- 管理后台地址:
  ...
  - 服务器直连:`http://<服务器IP>:8080/`
  - 对外访问(推荐):`https://<你的域名>/`
- **本项目不内置任何默认账号密码**,首次使用请访问 `http://<服务器IP>:8080/register` 注册;
- 采用 Bootstrap 机制: **首位注册用户自动获得 `admin` 角色**,后续注册为普通用户;
- 完成首位 admin 注册后,建议在**管理后台 → 系统设置**关闭"允许开放注册";
- 若你在公网暴露服务,请直接使用强密码注册,不要依赖固定默认口令;
- 忘记管理员密码或需提权,见「FAQ · 管理员密码找回 / 提权」。
...
**个人中心 → 在线体验 → 文生图 / 图生图**,左侧表单「**输出尺寸**」单选:原图 / 2K 高清 / 4K 高清,默认原图。切换后重新生成的图代理 URL 会自动走对应超分档位。
