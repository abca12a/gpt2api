...
- 用户收费按“请求档位”预扣并固化快照：`1k=0.06`、`2k=0.10`、`4k=0.20` 元；不要按号池最终命中的 provider 动态改价。
- 创建任务时落库 `requested_resolution`、`quoted_price`、`pricing_version`、`image_count`，结算/退款按创建时快照执行，避免中途改价导致账实不一致。
...
- `2k`：前端显示 `0.10`，后端预扣 `0.10`，号池优先 Codex/APIMart；外置失败可 fallback，成功仍结算 `0.10`。
- `4k`：前端显示 `0.20`，后端预扣 `0.20`，号池优先 Codex/APIMart；重参考图请求使用更长超时窗口，fallback 成功仍结算 `0.20`。
