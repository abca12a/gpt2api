...
- 2026-04-26 已按 han 要求增加图片渠道 502 Free 兜底：外置 Codex/OpenAI 兼容 image channel 在重试后仍返回 HTTP 502 / bad gateway 时，会转入内置 ChatGPT Web Runner，并强制调度 `plan_type=free` 账号；内容安全、400 参数错误等非 502 不兜底，避免绕过安全策略或掩盖真实参数问题。
...
- 2026-04-26 继续修复图片渠道偶发断流：外置 image channel（含同步、异步、chat->image）现在遇到 `502/5xx`、`stream disconnected before completion`、超时、EOF、connection reset 等瞬态错误时，会先在同一渠道上自动重试 1 次，再决定标记 unhealthy、切下一个渠道或任务失败；`400 invalid_value`、内容安全拦截等非瞬态错误不会误重试。这样可以压掉一部分单次断流造成的误失败，同时保留原有错误详情落库。
