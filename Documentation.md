...
- 2026-05-01（Asia/Shanghai）：下游后台 `gpt-image-2` 配置修复。`new-api` 后台不应把 `gpt-image-2:1k`、`gpt-image-2:2k`、`gpt-image-2:4k` 当作独立模型；真实模型列表、渠道和前端选择只保留 `gpt-image-2`。`ModelPrice` 中的 `gpt-image-2:1k/2k/4k` 是请求体分辨率价格键，由下游后端按请求体参数 `resolution`、`image_size`、`scale`、`quality`、`size` 归一化后读取。已在下游后端补回 `models.model_name='gpt-image-2'` 元信息并重启 `new-api-local`，`/api/pricing` 已验证返回 `resolution_options`：`1k=0`、`2k=0.06`、`4k=0.12`，`pricing_version=gpt-image-2-resolution-v1`。后台通用“请求规则计费”倍率适合折扣/倍数场景，不适合表达同一图片模型不同分辨率的固定价格；该模型优先使用现有分辨率价格键和请求体解析逻辑。
