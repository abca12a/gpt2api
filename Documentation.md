...
- 2026-05-01（Asia/Shanghai）：本轮重新确认 `gpt-image-2` 分档联动计划。历史 `model_price=0.12` 视为旧单价状态，不再作为本轮定价依据；目标用户价为 `1k=0.06`、`2k=0.10`、`4k=0.20` 元。默认缺失 resolution 的旧请求按 `1k` 兼容。`2k/4k` 成功时按请求档位结算，即使最终由 free runner 兜底；但 Web/free runner 只作为完成率兜底，不承诺原生稳定 2K/4K，原生大图能力仍以 Codex/APIMart 外置渠道为主。
