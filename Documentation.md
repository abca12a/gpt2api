...
- 2026-04-25 已针对“用户出图慢/长时间等待后失败”做快速换号优化：无参考图生图默认最多 3 次尝试、单次 90 秒、Poll 60 秒、调度等待 10 秒；如果 SSE 已结束但没有 `image_gen_task_id` 且没有任何图片引用，判定为上游未真正受理生图任务，立即暂停该账号并换号，不再空等 Poll 超时。
