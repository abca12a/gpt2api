...
### 2026-04-24 修复后台生成记录预览图加载失败

- 结论：后台“生成记录”预览弹窗仍直接返回 `image_tasks.result_urls` 里的上游临时直链，图片刷新后容易因过期、鉴权缺失或防盗链显示“加载失败”。
- 决策：新增统一的任务图片展示 URL 构造逻辑；只要任务存在 `file_ids`，后台生成记录和个人图片任务都返回本站 `/p/img/<task_id>/<idx>` 签名代理 URL；仅极老任务缺少 `file_ids` 时才兜底旧直链。
- 影响：后台生成记录的预览、打开原图都会复用现有账号鉴权代理下载链路，不再依赖短期上游直链；个人图片任务接口也复用同一逻辑，避免两边再次漂移。
- 验证：新增 `internal/image/proxy_url_test.go` 覆盖代理优先与旧直链兜底；`go test ./...` 通过。
